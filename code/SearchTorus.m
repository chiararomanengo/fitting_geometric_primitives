function [output, mfe] = SearchTorus(X)
...
if norm([0 0 1]'-n_bf)<10^(-2) || norm([0 0 1]'+n_bf)<0.2
    R=eye(3);
else
...
while size(C,1)<10
    for t=1:size(vVert,1)
        ...
        if norm([a b c])>10^(-3)
            ...
            if norm([0 0 1]-vPlane)<10^(-2) || norm([0 0 1]+vPlane)<0.2
                R=eye(3);
            else
                R = rot_mat([0;0;1],vPlane');
            end
            ...
            if (abs(min(xy(:,2)))<=0.2*max(xy(:,2)) || abs(max(xy(:,2)))<=0.2*abs(min(xy(:,2)))) % caso mezzo toro
                corda=max(xy(:,1))-min(xy(:,1));
                f=(max(xy(:,2))-min(xy(:,2)));
                if f < 0.1*corda
                    r=1.2*((corda^2/(8*f))+f/2);
...
[coord, maxCoord]=PlanesHT0_search(C);
...
if norm(ax-vPlaneTot)<0.6 || norm(ax+vPlaneTot)<0.6
    ax=vPlaneTot;
end

vPlane=ax;

if norm([0 0 1]-vPlane)<10^(-2) || norm([0 0 1]+vPlane)<10^(-2)
    R=eye(3);
else
    R = rot_mat([0;0;1],vPlane');
end
...
if max_H<2
    ...
    if maxCoord>max_H
        ...
        for k=1:numel(coord)
            ...
            circ=zeros(numel(theta),3);
            circ(:,1)=a1*cos(theta);
            circ(:,2)=a1*sin(theta); % da correggere con a e b
            [I,dist] = knnsearch(circ,xyz);
            m=MFE(xyz,dist);
            if m<mAus
                a=coord(k);
